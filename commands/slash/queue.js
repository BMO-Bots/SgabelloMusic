...
const command = new SlashCommand()
	...
	.setRun(async (client, interaction, options) => {
		...
		if (!player.queue.size || player.queue.size === 0) {
			...
			const queueEmbed = new MessageEmbed()
				.setColor(client.config.embedColor)
				.setDescription(`**♪ | Sto riproducendo:** [${ title }](${ song.uri })`)
				.addFields(
					{
						name: "Durata",
						value: song.isStream
			...
			let page = interaction.options.getNumber("Pagina");
			if (!page) {
				page = 0;
			}
			if (page) {
			...
			if (player.queue.size < 11 || player.queue.totalSize < 11) {
				...
				const embedTwo = new MessageEmbed()
					.setColor(client.config.embedColor)
					.setDescription(
						`**♪ | Riproducendo:** [${ title }](${ song.uri }) [${ player.queue.current.requester }]\n\n**Canzoni in coda:**\n${ pages[page] }`,
					)
					.addFields(
						{
							name: "Durata tracce",
					...
					.setFooter({
						text: `Pagina ${ page + 1 }/${ pages.length }`,
					});
				...
				const embedThree = new MessageEmbed()
					.setColor(client.config.embedColor)
					.setDescription(
						`**♪ | Ora riproducendo:** [${ title }](${ song.uri }) [${ player.queue.current.requester }]\n\n**Canzoni in coda:**\n${ pages[page] }`,
					)
					.addFields(
						{
							name: "Durata tracce",
				...
				const collector = interaction.channel.createMessageComponentCollector({
					filter: (b) => {
						if (b.user.id === interaction.user.id) {
							...
							return b
								.reply({
									content: `Solo **${ interaction.user.tag }** può usare questo tasto.`,
									ephemeral: true,
								})
								.catch(() => {
								});
