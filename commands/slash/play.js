...
const command = new SlashCommand()
  .setName("play")
  .setDescription(
    "Cerca e riproduce una canzone \nSuppporta: \nYoutube, Spotify, Deezer, Soundcloud"
  )
  .addStringOption((option) =>
    option
      .setName("query")
      .setDescription("Nome della canzone")
  ...
  .setRun(async (client, interaction, options) => {
    ...
    const ret = await interaction.reply({
      embeds: [
        new MessageEmbed()
          ...
          .setDescription(":mag_right: **Sto cercando...**"),
      ],
      fetchReply: true,
    });
      ...
      await interaction
        .editReply({
          embeds: [
            new MessageEmbed()
              .setColor("RED")
              .setDescription("Errore duurante la ricerca"),
          ],
        })
        .catch(this.warn);
    }
      ...
      await interaction
        .editReply({
          embeds: [
            new MessageEmbed()
              .setColor("RED")
              .setDescription("Non ho trovato risultati"),
          ],
        })
        .catch(this.warn);
    }
    ...
    if (res.loadType === "TRACK_LOADED" || res.loadType === "SEARCH_RESULT") {
      ...
      let addQueueEmbed = new MessageEmbed()
        .setColor(client.config.embedColor)
        .setAuthor({ name: "Aggiunto alla coda", iconURL: client.config.iconURL })
        .setDescription(
    ...
    if (res.loadType === "PLAYLIST_LOADED") {
      ...
      let playlistEmbed = new MessageEmbed()
        .setColor(client.config.embedColor)
        .setAuthor({
          name: "Playlist aggiunta alla coda",
        ...
        .addFields(
          {
            name: "Aggiunte in coda",
            value: `\`${res.tracks.length}\` songs`,
            inline: true,
          },
          {
            name: "Durata playlist",
